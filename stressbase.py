...
Example usage:
    python3 gold4.py --sweep 20:25 --count 10000 --seed 1
    python3 gold4.py --sweep 20:25 --count 10000 --seed 1 --quiet
...
Behavior:
    ...
    - Output per digit length:
        * Default (verbose): original detailed output.
        * With --quiet:
            D: _ | Seconds: _ | Ms per n: _ | S1 Wins vs S2 Wins: _ | Skip: _
"""
...
def run_sweep(start_digits: int, end_digits: int, count_per_digit: int, seed: int | None, quiet: bool) -> None:
    """
    ...
    for D in range(start_digits, end_digits + 1):
        ...
        if quiet:
            # Quiet mode: minimal per-digit line only
            print(
                f"D: {D} | Seconds: {seconds:.3f} | Ms per n: {ms_per_n:.6f} | "
                f"S1 Wins vs S2 Wins: {digit_s1_wins} vs {digit_s2_wins} | Skip: {digit_skip}"
            )
        else:
            # Verbose mode: full original output
            print(
                f"D: {D} | "
                f"Ms: {elapsed_ms:.3f} | "
    ...
    # Global summary only in verbose mode
    if not quiet:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Quiet mode: only print minimal per-digit stats, hide histograms and global summary.",
    )
